...
#pragma warning(suppress: 26495)
	...
	inline void start(void) noexcept {
		...
		_process_total_time = query.add_counter(L"\\Process(chat-server)\\% Processor Time");
		_process_user_time = query.add_counter(L"\\Process(chat-server)\\% User Time");
		_process_kernel_time = query.add_counter(L"\\Process(chat-server)\\% Privileged Time");
		...
		_process_private_byte = query.add_counter(L"\\Process(chat-server)\\Private Bytes");
		_process_pool_nonpaged_byte = query.add_counter(L"\\Process(chat-server)\\Pool Nonpaged Bytes");
...
private:
	...
	inline int monit(void) noexcept {
		...
		printf("--------------------------------------\n"\
			...
			message_pool._stack._capacity, message_pool._use_count, view_pool._stack._capacity, view_pool._use_count, _do_send_max);
	...
	inline void do_send_session(unsigned long long key, session::view_pointer& view_ptr) noexcept {
		...
		if (session_.acquire(key)) {
			if (512 > session_._send_queue.size()) {
				...
				session_.cancel();
				_InterlockedIncrement(&_do_send_max);
			}
		}
		if (session_.release())
...
public:
	...
	size_type _do_send_max = 0;
	size_type _do_send_max2 = 0;
