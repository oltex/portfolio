...
#pragma warning(suppress: 26495)
	...
	inline void start(void) noexcept {
		...
		_process_total_time = query.add_counter(L"\\Process(login-chat-server)\\% Processor Time");
		_process_user_time = query.add_counter(L"\\Process(login-chat-server)\\% User Time");
		_process_kernel_time = query.add_counter(L"\\Process(login-chat-server)\\% Privileged Time");
		...
		_process_private_byte = query.add_counter(L"\\Process(login-chat-server)\\Private Bytes");
		_process_pool_nonpaged_byte = query.add_counter(L"\\Process(login-chat-server)\\Pool Nonpaged Bytes");
